...
<%@ page import="com.myQuiz.quiz.Users"%>
<%@ page import="com.myQuiz.quiz.ResultModel"%>
<%@ page import="com.myQuiz.quiz.Helper"%>
...
<rows> <%
 	...
 	StringBuilder sql = new StringBuilder("SELECT ID ,username ,email,mobile,address " + " FROM  users");
 	sql.append(" order by id desc");
 	if (nRowsPerPage != 0) {
 		sql.append(" limit ").append(start).append(" , ").append(nRowsPerPage);
 	}
...
<%
	for (Users obj : list) {
%><row id="<%=obj.getId()%>"> <cell><![CDATA[<%=obj.getId()%>]]></cell>
<cell><![CDATA[<%=obj.getUsername()%>]]></cell> <cell><![CDATA[<%=obj.getEmail()%>]]></cell>
<cell><![CDATA[<%=obj.getMobile()%>]]></cell> <cell><![CDATA[<%=obj.getAddress()%>]]></cell>
<cell><![CDATA[<%="http://localhost:5526/OnlineQuizTest/dowImg?id=" + obj.getId()%>]]></cell>
